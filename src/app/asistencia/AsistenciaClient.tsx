...
import { buscarClientesAsistencia, registrarAsistenciaClienteAction, getAsistenciaHoy, registrarSalidaClienteAction, getNombreCliente } from '@/lib/supabase/actions/asistencia'
...
export default function AsistenciaClient({ initialRegistros, initialGymInfo }: { initialRegistros: any[], initialGymInfo: any }) {
  ...
  const [gymInfo, setGymInfo] = useState<any>(initialGymInfo)
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          try {
            const nombreCliente = await getNombreCliente(payload.new.cliente_id)
            
            showPremiumToast.success(
              'Ingreso Detectado', 
              `📢 ${nombreCliente || 'Un socio'} acaba de ingresar a la sala.`
            )
          } catch (e) {
            console.error('Error fetching client name on insert:', e)
            showPremiumToast.success('Ingreso Detectado', 'Se ha registrado un nuevo ingreso a la sala.')
          }
          fetchRegistros()
        }
      )
      .on(
        ...
        async (payload) => {
          ...
          try {
            if (payload.new.fecha_hora_salida) {
              const nombreCliente = await getNombreCliente(payload.new.cliente_id)
              
              showPremiumToast.info(
                'Salida Detectada', 
                `🚪 ${nombreCliente || 'Un socio'} ha registrado su salida de la sala.`
              )
            }
          } catch (e) {
            console.error('Error fetching client name on update:', e)
          }
          fetchRegistros()
        }
      )
  ...
  const { execute: executeRegistrar, isExecuting: isRegistrando } = useAction(registrarAsistenciaClienteAction, {
    onSuccess: ({ data: response }) => {
      if (response && response.success && response.data) {
        ...
        setResultadoIngreso({
          nombre: lastAuthorizedName,
          documento: '', // Need to pass or fetch
          ...response.data,
        })
        if (response.data.esCumpleanos) {
          showPremiumToast.success('¡Feliz Cumpleaños! 🎂', `¡Hoy es el día especial! Que tenga un excelente entrenamiento.`)
        } else if (response.data.esReingreso) {
          showPremiumToast.success('Re-ingreso Autorizado', `Socio ha vuelto a entrar a la sala.`)
        } else {
          showPremiumToast.success('Ingreso Exitoso', `Socio ha ingresado. Le quedan ${response.data.diasRestantes} días en su plan.`)
        }
        ...
        setErrorIngreso(response?.error || 'Error desconocido')
        showPremiumToast.error('Acceso Denegado', `No se pudo autorizar: ${response?.error}`)
      }
    },
    onError: ({ error }) => {
  ...
  const { execute: executeSalida, isExecuting: isRegistrandoSalida } = useAction(registrarSalidaClienteAction, {
    onSuccess: ({ data: response }) => {
      if (response && response.success) {
        showPremiumToast.success('Salida Exitosa', `Se ha registrado la salida correctamente.`)
        fetchRegistros()
      } else {
        showPremiumToast.error('Error en Salida', response?.error || 'Error desconocido')
      }
    },
    onError: () => {
  ...
  const handleRegistrar = async (cliente: any) => {
    ...
    setErrorIngreso(null)
    setResultadoIngreso(null)
    executeRegistrar({ clienteId: cliente.id })
    setRegistrando(null)
  }

  const handleRegistrarSalidaWrapper = async (asistenciaId: string, clienteNombre?: string) => {
    executeSalida({ asistenciaId })
  }
      ...
      <div className="max-w-5xl mx-auto space-y-10 pb-20 animate-in-fade">
                            ...
                            <TableCell>
                              {r.fecha_hora_salida ? (
                                ...
                                <Button 
                                  ...
                                  onClick={() => handleRegistrarSalidaWrapper(r.id, r.clientes?.nombre)}
                                >
                                  Marcar Salida
                                </Button>
                              )}
        ...
        {showSuccessOverlay && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none">
            <div className="bg-emerald-500 text-white px-6 md:px-12 py-6 md:py-8 rounded-[3rem] shadow-[0_20px_50px_rgba(16,185,129,0.4)] animate-in zoom-in-90 fade-in duration-300 flex flex-col items-center gap-4 border-4 border-white/20 backdrop-blur-md mx-4 max-w-[90vw]">
