      ...
      <div className="space-y-6 md:space-y-10 pb-20">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between p-1">
          <div className="flex gap-3">
            {[1, 2, 3].map(i => (
              <Skeleton key={i} className="h-9 w-28 bg-white/5 rounded-full" />
            ))}
          </div>
          <Skeleton className="h-11 w-full md:w-96 bg-white/5 rounded-full" />
        </div>
            ...
            {[...Array(8)].map((_, i) => (
              <div key={i} className="p-5 flex items-center justify-between hover:bg-white/[0.01] transition-colors">
                ...
                <div className="flex items-center gap-4">
                  <Skeleton className="h-9 w-9 bg-white/5 rounded-lg" />
                  <Skeleton className="h-9 w-9 bg-white/5 rounded-lg" />
                </div>
              </div>
            ))}
