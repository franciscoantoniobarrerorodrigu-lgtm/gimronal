...
import { updatePasswordAction, updateUserPasswordAsAdminAction } from '@/lib/supabase/actions/auth'
...
export default function SeguridadClient({ initialUsers }: { initialUsers: any[] }) {
  ...
  const { execute: exeUpdatePwd, isExecuting: loadingPwd } = useAction(updatePasswordAction, {
    onSuccess: ({ data: res }) => {
      if (res?.success) {
  ...
  const { execute: exeAdminReset, isExecuting: loadingAdmin } = useAction(updateUserPasswordAsAdminAction, {
    onSuccess: ({ data: res }) => {
      if (res?.success) {
  ...
  const handleAdminResetPassword = async () => {
    ...
    exeAdminReset({ userId: selectedUser.id, newPassword: resetPassword })
  }
        ...
        <Dialog open={isResetDialogOpen} onOpenChange={setIsResetDialogOpen}>
          <DialogContent className="bg-zinc-950 border-zinc-800 text-white">
            ...
            <DialogFooter>
              ...
              >
                {loadingAdmin ? 'Guardando...' : 'Cambiar Contraseña'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
