...
import { getPagos, eliminarPago } from '@/lib/supabase/actions/pagos'
...
export default function PagosPage() {
  ...
  const [periodFilter, setPeriodFilter] = useState<'hoy' | 'semana' | 'mes' | 'todos'>('hoy')
  ...
  const stats = useMemo(() => {
    ...
    const ayerStr = getColombiaDateString() // Simplificado, luego calculamos ayer real
  ...
  async function handleEliminarPago(pago: any) {
    const confirmacion = window.confirm('¿Estás seguro de eliminar este pago? También se anulará la membresía asociada.');
    if (confirmacion) {
      try {
        ...
        showPremiumToast.error('Error de Comunicación', 'No se pudo procesar la solicitud de eliminación en este momento.');
      }
    }
  }
  ...
  return (
    <AdminLayout 
      isGymActive={gimnasio?.activo !== false} 
      vencimientoLicencia={gimnasio?.vencimiento_licencia}
        ...
        <DialogContent className="glass-card border-white/10 sm:max-w-[425px]">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button 
              ...
              onClick={() => {
                if (pagoToDelete) {
                  handleEliminarPago(pagoToDelete)
